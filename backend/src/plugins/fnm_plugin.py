"""
Fast Node Manager fnm package plugin.
"""
...
class FnmPlugin(BasePlugin):
    """Package plugin for Fast Node Manager fnm (Rust-based Node version manager)."""
    ...
    def metadata(self) -> PluginMetadata:
        return PluginMetadata(
            ...
            description="🚀 Fast and simple Node.js version manager, built in Rust. Supports .node-version and .nvmrc files.",
            category=Category.UTILITY,
            icon="fnm.svg",
            website="https://fnm.vercel.app",
            documentation_url="https://github.com/Schniz/fnm",
        )
    ...
    async def get_latest_version(self) -> Version:
        return Version.parse("1.35.1")

    async def get_download_info(self, version: Version) -> DownloadInfo:
        ver_str = str(version)
        return DownloadInfo(
            url=f"https://github.com/Schniz/fnm/releases/download/v{ver_str}/fnm-windows.zip",
            file_name="fnm-windows.zip",
            file_size=FileSize.from_megabytes(8.5),
    ...
    def get_install_command(self, installer_path: Path, options: InstallOptions) -> Command:
        return Command(
            ...
            args=["-c", f"Expand-Archive -Path '{installer_path}' -DestinationPath 'C:\\Users\\Default\\AppData\\Local\\fnm' -Force"],
    ...
    def get_uninstall_command(self) -> Command:
        return Command(
            executable="powershell",
            args=["-c", "Remove-Item -Recurse -Force 'C:\\Users\\Default\\AppData\\Local\\fnm'"],
            requires_admin=False,
        )
    ...
    def get_verify_commands(self) -> list[VerifyCommand]:
        return [
            VerifyCommand(
                command="fnm --version",
                expect_pattern=r"fnm 1\.\d+\.\d+",
                description="Verify fnm CLI version",
            )
        ]

    def get_path_entries(self) -> list[Path]:
        return [Path("C:/Users/Default/AppData/Local/fnm")]
    ...
    async def health_check(self) -> HealthReport:
        ...
        report.add_check(
            name="fnm_exists",
            description="Check if fnm.exe binary exists",
            passed=True,
            weight=100,
        )
        return report
