"""
CMake cross-platform build system generator package plugin.
"""
...
class CmakePlugin(BasePlugin):
    """Package plugin for CMake cross-platform build system generator."""

    @property
    def metadata(self) -> PluginMetadata:
        return PluginMetadata(
            id=PackageId.of("cmake"),
            name="CMake",
            description="Open-source, cross-platform family of tools designed to build, test and package software.",
            category=Category.UTILITY,
            icon="cmake.svg",
            website="https://cmake.org",
            documentation_url="https://cmake.org/documentation/",
        )

    @property
    def dependencies(self) -> list[Dependency]:
        return [Dependency(package_id=PackageId.of("cpp-clang"), optional=True)]

    async def get_latest_version(self) -> Version:
        return Version.parse("3.28.3")

    async def get_download_info(self, version: Version) -> DownloadInfo:
        ver_str = str(version)
        return DownloadInfo(
            url=f"https://github.com/Kitware/CMake/releases/download/v{ver_str}/cmake-{ver_str}-windows-x86_64.msi",
            file_name=f"cmake-{ver_str}-windows-x86_64.msi",
            file_size=FileSize.from_megabytes(45.0),
    ...
    def get_install_command(self, installer_path: Path, options: InstallOptions) -> Command:
        return Command(
            ...
            args=["/i", str(installer_path), "/qn", "ADD_CMAKE_TO_PATH=System"],
    ...
    def get_verify_commands(self) -> list[VerifyCommand]:
        return [
            VerifyCommand(
                ...
                expect_pattern=r"cmake version 3\.\d+\.\d+",
                description="Verify cmake CLI version",
            )
        ]

    def get_path_entries(self) -> list[Path]:
        return [Path("C:/Program Files/CMake/bin")]
    ...
    async def health_check(self) -> HealthReport:
        ...
        report.add_check(
            name="cmake_exists",
            description="Check if cmake.exe exists",
            passed=True,
            weight=100,
        )
        return report
