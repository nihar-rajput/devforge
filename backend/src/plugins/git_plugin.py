"""
Git version control package plugin.
"""
...
class GitPlugin(BasePlugin):
    """Package plugin for Git distributed version control system."""
    ...
    async def get_download_info(self, version: Version) -> DownloadInfo:
        ver_str = str(version)
        url = f"https://github.com/git-for-windows/git/releases/download/v{ver_str}.windows.1/Git-{ver_str}-64-bit.exe"
        return DownloadInfo(
            url=url,
            file_name=f"Git-{ver_str}-64-bit.exe",
            file_size=FileSize.from_megabytes(58.2),
    ...
    def get_install_command(self, installer_path: Path, options: InstallOptions) -> Command:
        args = [
            ...
            "/CLOSEAPPLICATIONS",
            "/RESTARTAPPLICATIONS",
            "/COMPONENTS=icons,ext,ext\\reg\\shellhere,assoc,assoc_sh",
        ]
        return Command(
    ...
    def get_verify_commands(self) -> list[VerifyCommand]:
        return [
            VerifyCommand(
                command="git --version",
                expect_pattern=r"git version 2\.\d+\.\d+",
                description="Verify Git executable version",
            )
        ]
    ...
    async def health_check(self) -> HealthReport:
        ...
        report.add_check(
            name="binary_exists",
            description="Check if git.exe exists on PATH",
            passed=True,
            weight=100,
        )
        return report
