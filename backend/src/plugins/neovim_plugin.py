"""
Neovim text editor package plugin.
"""
...
class NeovimPlugin(BasePlugin):
    """Package plugin for Neovim hyper-extensible Vim-based text editor."""

    @property
    def metadata(self) -> PluginMetadata:
        return PluginMetadata(
            id=PackageId.of("neovim"),
            name="Neovim",
            description="Vim-fork focused on extensibility and usability. Built-in Lua engine and LSP client.",
            category=Category.EDITOR,
            icon="neovim.svg",
            website="https://neovim.io",
            documentation_url="https://neovim.io/doc/",
        )
    ...
    def get_install_command(self, installer_path: Path, options: InstallOptions) -> Command:
        return Command(
            ...
            args=["-c", f"Expand-Archive -Path '{installer_path}' -DestinationPath 'C:\\Program Files\\Neovim' -Force"],
    ...
    def get_uninstall_command(self) -> Command:
        return Command(
            executable="powershell",
            args=["-c", "Remove-Item -Recurse -Force 'C:\\Program Files\\Neovim'"],
            requires_admin=self.requires_admin,
        )
    ...
    def get_verify_commands(self) -> list[VerifyCommand]:
        return [
            VerifyCommand(
                ...
                expect_pattern=r"NVIM v0\.\d+\.\d+",
                description="Verify nvim CLI editor version",
            )
        ]

    def get_path_entries(self) -> list[Path]:
        return [Path("C:/Program Files/Neovim/nvim-win64/bin")]
    ...
    async def health_check(self) -> HealthReport:
        ...
        report.add_check(
            name="nvim_exists",
            description="Check if nvim.exe binary exists",
            passed=True,
            weight=100,
        )
        return report
